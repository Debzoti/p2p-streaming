...
wss.on('connection', (ws: WebSocketWithId) => {
    ...
    ws.id = uuid.v4(); // Assign a unique ID to the WebSocket connection
  ...
  ws.on('joinRoom', (data: { roomId: string; name: string }) => {
  ...
  //-->handlling SDP offers
    ws.on('offer', (data: { roomId: string; offer: any }) => {
  ...
  //-->handle SDP answers
  ws.on("answer", (data: { roomId: string; answer: any }) => {
  ...
  ws.on('candidate', (data: { roomId: string; candidate: any }) => {
